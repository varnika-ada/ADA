...
with open("kthsmallest_output.txt", "r") as file:
    for line in file:
...
plt.plot(array_size, duration,
         marker='o',
         linewidth=3,
         label="Actual Time")

plt.plot(array_size, best_case,
         linestyle='--',
         linewidth=2,
         label="Best Case O(n)")

plt.plot(array_size, avg_case,
         linestyle='-.',
         linewidth=2,
         label="Average Case O(n)")

plt.plot(array_size, worst_case,
         linestyle=':',
         linewidth=3,
...
plt.ylabel("Duration")
plt.title("Kth Smallest Element using Quick Select")
plt.legend()
plt.grid(True)
plt.show()